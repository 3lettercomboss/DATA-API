...
app.post("/api/adonis-logs", async (req, res) => {
  try {
    ...
    if (!playerId || !command) return res.status(400).json({ ok: false, error: "playerId and command are required" });
    ...
    // Check staff table
    const staffResult = await pool.query("SELECT username, discord_id, rank FROM staff WHERE player_id = $1", [playerId]);
    if (staffResult.rows.length > 0) {
      username = staffResult.rows[0].username;
      discordId = staffResult.rows[0].discord_id;
    }
    ...
    // If they're in staff table, log as staff
